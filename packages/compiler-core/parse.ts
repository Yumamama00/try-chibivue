...
function parseChildren(
  ...
  while (!isEnd(context, ancestors)) {
    ...
    if (!node) {
      // 上記の条件に当てはまらなかった場合位はTextNodeとしてパース
      node = parseText(context);
    }

    pushNode(nodes, node as TemplateChildNode);
  }
...
function isEnd(context: ParserContext, ancestors: ElementNode[]): boolean {
  ...
  // sが"</"で始まり、かつその後にancestorsのタグ名が続くことを判定し、
  // 閉じタグがあるか(parseChildrenが終了するべきか)を判定
  if (startsWith(s, "</")) {
...
// -------------------- 以下からはユーティリティ (parseElementなどでも使う) --------------------
...
// 属性のvalueをパース
// valueのクォートはシングルでもダブルでもパースできるように実装している
